...
const IAControlPanel = () => { //Definición de variables de control
  const [loadingTrain, setLoadingTrain] = useState(false);
  const [searchId, setSearchId] = useState(''); //Almacén de ID
  const [prediccion, setPrediccion] = useState(null);
  const [loadingPredict, setLoadingPredict] = useState(false);
  const [error, setError] = useState(null); // Para mensaje de error

  // 1. ENTRENAR
  const handleTrain = async () => {
    setLoadingTrain(true);
    try {
      const res = await fetch('http://127.0.0.1:8000/ml/entrenar', { method: 'POST' }); // Petición al backend
      if (!res.ok) throw new Error('Error en entrenamiento');
      const data = await res.json();
      alert(`✅ ${data.message || "Modelo entrenado"}`); // Control de errores
    } catch (err) {
  ...
  const handlePredict = async (e) => {
    ...
    if (!searchId) return; // tiene que haber un ID
    setLoadingPredict(true);
    setError(null);
    setPrediccion(null);

    try {
      // Petición al backend (llamada al endpoint)
      ...
      const misDatos =  data.metricas  
      const datosGrupo = data.comparativa_grupo || {};

      // 3. Convertimos los datos para el formato de la gráfica
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-indigo-100 overflow-hidden mb-10">
      ...
      <div className="bg-gradient-to-r from-violet-600 to-indigo-600 p-6 text-white">
        ...
        <p className="text-indigo-100 opacity-80 text-sm">Compara tus métricas con la media de tu perfil</p>
      </div>
          ...
          <div>
            ...
            <form onSubmit={handlePredict} className="flex gap-2">
              <input 
                ...
                onChange={(e) => setSearchId(e.target.value)} //Actualiza la variable 
