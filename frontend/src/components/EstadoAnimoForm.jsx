...
const EstadoAnimoForm = () => {
  const [datos, setDatos] = useState({ puntuacion_dia: 7, nivel_energia: 7, notas: '' });

  const handleChange = (e) => setDatos({ ...datos, [e.target.name]: e.target.value });
  const handleSubmit = (e) => { e.preventDefault(); alert(`Ánimo: ${JSON.stringify(datos)}`); };
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-lg border border-yellow-100">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" className="w-full bg-yellow-500 text-white font-bold py-2 rounded mt-4 hover:bg-yellow-600">GUARDAR</button>
      </form>
    </div>
  );
};
